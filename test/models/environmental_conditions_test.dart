...
void main() {
  group('EnvironmentalConditions 상세 테스트', () {
    ...
    group('발효 속도 온도 보정 테스트', () {
      ...
      test('중간 온도(30°C)에서 발효 속도 보정이 정확해야 함', () {
        ...
        expect(
            warmConditions.fermentationSpeedCorrection, closeTo(1.414, 0.001));
      });
    });
    ...
    group('계절별 특성 테스트', () {
      ...
      test('계절 변경이 정상 동작해야 함', () {
        final summerConditions =
            standardConditions.copyWith(season: Season.summer);
    ...
    group('JSON 직렬화 테스트', () {
      test('환경 조건 JSON 직렬화가 정상 동작해야 함', () {
        ...
        expect(standardConditions.oven.type,
            equals(OvenType.professionalConvection));
      });
    ...
    group('데이터 유효성 검증 테스트', () {
      test('온도 범위 검증', () {
        ...
        expect(() => standardConditions.copyWith(temperature: 10.0),
            returnsNormally);
        expect(() => standardConditions.copyWith(temperature: 40.0),
            returnsNormally);

        // 극한 온도도 허용 (계산은 가능)
        expect(() => standardConditions.copyWith(temperature: -10.0),
            returnsNormally);
        expect(() => standardConditions.copyWith(temperature: 60.0),
            returnsNormally);
      });
      ...
      test('습도 범위 검증', () {
        // 일반적인 습도 범위: 30% ~ 90%
        expect(
            () => standardConditions.copyWith(humidity: 30.0), returnsNormally);
        expect(
            () => standardConditions.copyWith(humidity: 90.0), returnsNormally);

        // 극한 습도도 허용
        expect(
            () => standardConditions.copyWith(humidity: 10.0), returnsNormally);
        expect(() => standardConditions.copyWith(humidity: 100.0),
            returnsNormally);
      });

      test('기압 범위 검증', () {
        // 일반적인 기압 범위: 950hPa ~ 1050hPa
        expect(() => standardConditions.copyWith(pressure: 950.0),
            returnsNormally);
        expect(() => standardConditions.copyWith(pressure: 1050.0),
            returnsNormally);
      });
      ...
      test('고도 범위 검증', () {
        // 일반적인 고도 범위: 0m ~ 3000m
        expect(
            () => standardConditions.copyWith(altitude: 0.0), returnsNormally);
        expect(() => standardConditions.copyWith(altitude: 3000.0),
            returnsNormally);

        // 음수 고도도 허용 (해수면 아래)
        expect(() => standardConditions.copyWith(altitude: -100.0),
            returnsNormally);
      });
    });
    ...
    group('copyWith 메서드 테스트', () {
      test('개별 속성 변경이 정상 동작해야 함', () {
        ...
        expect(
            modified.pressure, equals(standardConditions.pressure)); // 변경되지 않음
  ...
  group('EnvironmentalConditionsValidator 테스트', () {
    ...
    group('개별 유효성 검증 테스트', () {
      test('유효한 온도 범위 검증', () {
        expect(
            EnvironmentalConditionsValidator.isValidTemperature(25.0), isTrue);
        expect(
            EnvironmentalConditionsValidator.isValidTemperature(10.0), isTrue);
        expect(
            EnvironmentalConditionsValidator.isValidTemperature(40.0), isTrue);
      });

      test('무효한 온도 범위 검증', () {
        expect(EnvironmentalConditionsValidator.isValidTemperature(-25.0),
            isFalse);
        expect(
            EnvironmentalConditionsValidator.isValidTemperature(65.0), isFalse);
      });
      ...
      test('무효한 습도 범위 검증', () {
        expect(
            EnvironmentalConditionsValidator.isValidHumidity(-10.0), isFalse);
        expect(
            EnvironmentalConditionsValidator.isValidHumidity(110.0), isFalse);
      });

      test('유효한 기압 범위 검증', () {
        expect(
            EnvironmentalConditionsValidator.isValidPressure(1013.0), isTrue);
        expect(EnvironmentalConditionsValidator.isValidPressure(900.0), isTrue);
        expect(
            EnvironmentalConditionsValidator.isValidPressure(1100.0), isTrue);
      });

      test('무효한 기압 범위 검증', () {
        expect(
            EnvironmentalConditionsValidator.isValidPressure(700.0), isFalse);
        expect(
            EnvironmentalConditionsValidator.isValidPressure(1300.0), isFalse);
      });

      test('유효한 고도 범위 검증', () {
        expect(EnvironmentalConditionsValidator.isValidAltitude(0.0), isTrue);
        expect(
            EnvironmentalConditionsValidator.isValidAltitude(1000.0), isTrue);
        expect(EnvironmentalConditionsValidator.isValidAltitude(null), isTrue);
      });

      test('무효한 고도 범위 검증', () {
        expect(
            EnvironmentalConditionsValidator.isValidAltitude(-600.0), isFalse);
        expect(
            EnvironmentalConditionsValidator.isValidAltitude(6000.0), isFalse);
      });
    });
    ...
    group('전체 환경 조건 유효성 검증 테스트', () {
      test('유효한 환경 조건은 검증을 통과해야 함', () {
        final result =
            EnvironmentalConditionsValidator.validateEnvironmentalConditions(
                validConditions);
    ...
    group('환경 권장사항 테스트', () {
      test('표준 환경에서는 기본 권장사항을 제공해야 함', () {
        final recommendation =
            EnvironmentalConditionsValidator.getRecommendations(
                validConditions);
      ...
      test('저온 환경에서는 발효 시간 연장 권장사항을 제공해야 함', () {
        ...
        final recommendation =
            EnvironmentalConditionsValidator.getRecommendations(coldConditions);

        expect(
            recommendation.recommendations.any((r) => r.contains('발효 시간이 길어질')),
            isTrue);
      });
      ...
      test('고온 환경에서는 발효 주의 권장사항을 제공해야 함', () {
        ...
        final recommendation =
            EnvironmentalConditionsValidator.getRecommendations(hotConditions);

        expect(recommendation.recommendations.any((r) => r.contains('발효가 빨라질')),
            isTrue);
      });
      ...
      test('저습도 환경에서는 덮개 사용 권장사항을 제공해야 함', () {
        ...
        final recommendation =
            EnvironmentalConditionsValidator.getRecommendations(dryConditions);

        expect(recommendation.recommendations.any((r) => r.contains('덮개를 사용')),
            isTrue);
      });
      ...
      test('고습도 환경에서는 곰팡이 주의 권장사항을 제공해야 함', () {
        ...
        expect(
            recommendation.recommendations.any((r) => r.contains('곰팡이 발생에 주의')),
            isTrue);
      });

      test('계절별 권장사항이 정확해야 함', () {
        final summerConditions =
            validConditions.copyWith(season: Season.summer);
        final winterConditions =
            validConditions.copyWith(season: Season.winter);

        final summerRec = EnvironmentalConditionsValidator.getRecommendations(
            summerConditions);
        final winterRec = EnvironmentalConditionsValidator.getRecommendations(
            winterConditions);
  ...
  group('OvenCharacteristicsValidator 테스트', () {
    test('유효한 오븐 특성은 검증을 통과해야 함', () {
      ...
      final result =
          OvenCharacteristicsValidator.validateOvenCharacteristics(validOven);
    ...
    test('무효한 유형 계수는 에러를 발생시켜야 함', () {
      ...
      final result =
          OvenCharacteristicsValidator.validateOvenCharacteristics(invalidOven);
    ...
    test('낮은 최대 온도는 경고를 발생시켜야 함', () {
      ...
      final result =
          OvenCharacteristicsValidator.validateOvenCharacteristics(lowTempOven);
    ...
    test('스팀 오븐의 낮은 스팀 능력은 경고를 발생시켜야 함', () {
      ...
      final result = OvenCharacteristicsValidator.validateOvenCharacteristics(
          steamOvenLowCapability);

...
void main() {
  group('BakingScienceFormulaEngine 기본 테스트', () {
    ...
    test('기본 재료 비율 계산이 정확해야 함', () {
      final result = BakingScienceFormulaEngine.optimizeMaterialRatio(basicBreadIngredients);
    ...
    test('환경 보정 계수 계산이 정확해야 함', () {
      ...
      final correction = BakingScienceFormulaEngine.calculateEnvironmentCorrection(standardConditions);
    ...
    test('글루텐 강도 지수 계산이 정확해야 함', () {
      final ratios = BakingScienceFormulaEngine.optimizeMaterialRatio(basicBreadIngredients);
      final metadata = [
      ...
      final doughState = BakingScienceFormulaEngine.calculateDoughState(ratios, metadata);
    ...
    test('굽기 공정 최적화가 정상 동작해야 함', () {
      ...
      expect(bakingProcess.donenessIndicators.internalTemperature, equals(95.0));
    });
  });
}
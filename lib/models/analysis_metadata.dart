...
class AnalysisMetadata {
  ...
  /// 경고 로그 개수
  int get warningCount => logs.where((log) => log.level == LogLevel.warning).length;
  ...
  factory AnalysisMetadata.fromJson(Map<String, dynamic> json) {
    return AnalysisMetadata(
      ...
      moduleExecutionTimes: Map<String, int>.from(json['moduleExecutionTimes'] as Map),
      memoryUsage: MemoryUsageInfo.fromJson(json['memoryUsage'] as Map<String, dynamic>),
      cacheStats: CacheStatistics.fromJson(json['cacheStats'] as Map<String, dynamic>),
...
class LogEntry {
  ...
  /// 팩토리 생성자들
  factory LogEntry.debug(String message, {String? moduleName, Map<String, dynamic>? context}) {
    return LogEntry(
  ...
  factory LogEntry.info(String message, {String? moduleName, Map<String, dynamic>? context}) {
    return LogEntry(
      level: LogLevel.info,
      message: message,
      timestamp: DateTime.now(),
  ...
  factory LogEntry.warning(String message, {String? moduleName, Map<String, dynamic>? context}) {
    return LogEntry(
  ...
  factory LogEntry.error(String message, {String? moduleName, Map<String, dynamic>? context}) {
    return LogEntry(
...
class PerformanceMetrics {
  ...
  factory PerformanceMetrics.calculate({
    ...
    final averageResponseTime = moduleExecutionTimes.isNotEmpty
        ? moduleExecutionTimes.values.reduce((a, b) => a + b) / moduleExecutionTimes.length.toDouble()
        : 0.0;
    ...
    // CPU 사용률 (추정치)
    final cpuUsage = processingTime > 0 ? (processingTime / 30000.0).clamp(0.0, 1.0) : 0.0;
    ...
    // 전체 성능 점수 계산
    final timeScore = processingTime <= 10000 ? 100.0 : (100.0 - (processingTime - 10000) / 1000.0).clamp(0.0, 100.0);
    final overallScore = (timeScore * 0.4 + memoryEfficiency * 0.3 + (100.0 - cpuUsage * 100.0) * 0.3);

...
class IngredientMetadata {
  ...
  IngredientMetadata copyWith({
    ...
    return IngredientMetadata(
      ...
      qualityCorrectionFactor: qualityCorrectionFactor ?? this.qualityCorrectionFactor,
    );
  }
  ...
  double get effectiveProteinPercentage {
    ...
    final activityIndex = (properties['activityIndex'] as num?)?.toDouble() ?? 1.0;
    return protein * activityIndex;
  }

  /// 유효 이스트 활성도% = 표준 이스트 활성도% × 이스트 신선도 지수 (0.5~1.0)
  double get effectiveYeastActivity {
    final standardActivity = (properties['standardActivity'] as num?)?.toDouble() ?? 0.0;
    final freshnessIndex = (properties['freshnessIndex'] as num?)?.toDouble() ?? 1.0;
...
class MixingMeta {
  ...
  MixingMeta copyWith({
    ...
    return MixingMeta(
      ...
      glutenDevelopmentTarget: glutenDevelopmentTarget ?? this.glutenDevelopmentTarget,
...
class FermentationMeta {
  ...
  FermentationMeta copyWith({
    ...
    return FermentationMeta(
      ...
      microbialActivityCoefficient: microbialActivityCoefficient ?? this.microbialActivityCoefficient,
      doughPhysicalChemicalCoefficient: doughPhysicalChemicalCoefficient ?? this.doughPhysicalChemicalCoefficient,
      environmentalClimateCoefficient: environmentalClimateCoefficient ?? this.environmentalClimateCoefficient,
...
class EnvironmentalConditionsValidator {
  ...
  /// 전체 환경 조건 유효성 검증
  static ValidationResult validateEnvironmentalConditions(EnvironmentalConditions conditions) {
    final errors = <String>[];
    final warnings = <String>[];

    // 온도 검증
    if (!isValidTemperature(conditions.temperature)) {
      errors.add('온도가 유효 범위(-20°C ~ 60°C)를 벗어났습니다: ${conditions.temperature}°C');
    } else if (conditions.temperature < 10.0 || conditions.temperature > 40.0) {
      warnings.add('온도가 일반적인 제빵 환경(10°C ~ 40°C)을 벗어났습니다: ${conditions.temperature}°C');
    }
    ...
    if (!isValidHumidity(conditions.humidity)) {
      ...
      warnings.add('습도가 일반적인 제빵 환경(30% ~ 90%)을 벗어났습니다: ${conditions.humidity}%');
    }

    // 기압 검증
    if (!isValidPressure(conditions.pressure)) {
      errors.add('기압이 유효 범위(800hPa ~ 1200hPa)를 벗어났습니다: ${conditions.pressure}hPa');
    } else if (conditions.pressure < 950.0 || conditions.pressure > 1050.0) {
      warnings.add('기압이 일반적인 범위(950hPa ~ 1050hPa)를 벗어났습니다: ${conditions.pressure}hPa');
    }

    // 고도 검증
    if (!isValidAltitude(conditions.altitude)) {
      errors.add('고도가 유효 범위(-500m ~ 5000m)를 벗어났습니다: ${conditions.altitude}m');
    } else if (conditions.altitude != null && (conditions.altitude! < 0.0 || conditions.altitude! > 3000.0)) {
      warnings.add('고도가 일반적인 범위(0m ~ 3000m)를 벗어났습니다: ${conditions.altitude}m');
    }

    // 오븐 특성 검증
    final ovenValidation = OvenCharacteristicsValidator.validateOvenCharacteristics(conditions.oven);
  ...
  /// 환경 조건 권장 범위 확인
  static EnvironmentalRecommendation getRecommendations(EnvironmentalConditions conditions) {
    ...
    return EnvironmentalRecommendation(
      ...
      optimalTemperatureRange: const TemperatureRange(min: 22.0, max: 28.0),
      optimalHumidityRange: const HumidityRange(min: 60.0, max: 75.0),
    );
  }
}
...
class OvenCharacteristicsValidator {
  /// 오븐 특성 유효성 검증
  static ValidationResult validateOvenCharacteristics(OvenCharacteristics oven) {
    final errors = <String>[];
    final warnings = <String>[];
    ...
    if (oven.calibrationIndex < 0.8 || oven.calibrationIndex > 1.5) {
      warnings.add('캘리브레이션 지수가 일반적인 범위(0.8 ~ 1.5)를 벗어났습니다: ${oven.calibrationIndex}');
    }
    ...
    if (oven.maxTemperature < 150.0 || oven.maxTemperature > 500.0) {
      errors.add('최대 온도가 유효 범위(150°C ~ 500°C)를 벗어났습니다: ${oven.maxTemperature}°C');
    } else if (oven.maxTemperature < 220.0) {
      warnings.add('최대 온도가 낮습니다(${oven.maxTemperature}°C). 일부 빵 종류에 제한이 있을 수 있습니다.');
    }
...
class IngredientAnalyzer {
  ...
  static List<IngredientFunction> analyzeFunctions(String ingredientName) {
    ...
    // 구조 형성 재료
    if (lowerName.contains('flour') || lowerName.contains('밀가루') || 
        lowerName.contains('강력분') || lowerName.contains('중력분') || 
        lowerName.contains('박력분')) {
      functions.add(IngredientFunction.structureFormation);
    }

    // 발효 재료
    if (lowerName.contains('yeast') || lowerName.contains('이스트') || 
        lowerName.contains('효모') || lowerName.contains('starter') || 
        lowerName.contains('스타터') || lowerName.contains('사워도우')) {
      functions.add(IngredientFunction.fermentation);
    }

    // 풍미 향상 재료
    if (lowerName.contains('salt') || lowerName.contains('소금') || 
        lowerName.contains('sugar') || lowerName.contains('설탕') || 
        lowerName.contains('honey') || lowerName.contains('꿀') ||
        lowerName.contains('vanilla') || lowerName.contains('바닐라')) {
      functions.add(IngredientFunction.flavorEnhancement);
    }

    // 텍스처 조절 재료
    if (lowerName.contains('butter') || lowerName.contains('버터') || 
        lowerName.contains('oil') || lowerName.contains('기름') || 
        lowerName.contains('올리브오일') || lowerName.contains('지방')) {
      functions.add(IngredientFunction.textureModification);
    }

    // 수분 조절 재료
    if (lowerName.contains('water') || lowerName.contains('물') || 
        lowerName.contains('milk') || lowerName.contains('우유') || 
        lowerName.contains('cream') || lowerName.contains('크림')) {
      functions.add(IngredientFunction.moistureControl);
    }

    // 팽창 재료
    if (lowerName.contains('baking powder') || lowerName.contains('베이킹파우더') || 
        lowerName.contains('baking soda') || lowerName.contains('베이킹소다') || 
        lowerName.contains('탄산수소나트륨')) {
      functions.add(IngredientFunction.leavening);
    }

    // 결합 재료
    if (lowerName.contains('egg') || lowerName.contains('계란') || 
        lowerName.contains('달걀') || lowerName.contains('gelatin') || 
        lowerName.contains('젤라틴')) {
      functions.add(IngredientFunction.binding);
    }

    // 착색 재료
    if (lowerName.contains('cocoa') || lowerName.contains('코코아') || 
        lowerName.contains('chocolate') || lowerName.contains('초콜릿') || 
        lowerName.contains('food coloring') || lowerName.contains('식용색소')) {
      functions.add(IngredientFunction.coloring);
    }

    // 장식 재료
    if (lowerName.contains('nuts') || lowerName.contains('견과류') || 
        lowerName.contains('fruit') || lowerName.contains('과일') || 
        lowerName.contains('raisin') || lowerName.contains('건포도')) {
  ...
  static double calculateQualityCorrectionFactor(Ingredient ingredient) {
    ...
    if (ingredient.isYeast) {
      if (lowerName.contains('instant') || lowerName.contains('인스턴트')) {
        factor *= 1.2; // 인스턴트 이스트
      } else if (lowerName.contains('active dry') || lowerName.contains('드라이')) {
        factor *= 1.0; // 드라이 이스트
...
class IngredientMetadataGenerator {
  ...
  static IngredientMetadata generateMetadata(Ingredient ingredient) {
    ...
    final qualityFactor = IngredientAnalyzer.calculateQualityCorrectionFactor(ingredient);
  ...
  static List<IngredientMetadata> generateMetadataList(List<Ingredient> ingredients) {
    return ingredients.map((ingredient) => generateMetadata(ingredient)).toList();
  }

  /// 재료 호환성 검사
  static CompatibilityResult checkIngredientCompatibility(List<Ingredient> ingredients) {
    final issues = <String>[];
    final warnings = <String>[];

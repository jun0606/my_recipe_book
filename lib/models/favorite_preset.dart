...
class FavoritePreset {
  ...
  factory FavoritePreset.fromJson(Map<String, dynamic> json) {
    return FavoritePreset(
      ...
      averageImprovementScore: (json['averageImprovementScore'] as num).toDouble(),
  ...
  FavoritePreset updateUsage({
    ...
    final newAvgImprovementScore = newImprovementScore != null
        ? (averageImprovementScore * (newUsageCount - 1) + newImprovementScore) / newUsageCount
  ...
  PresetQuality get quality {
    ...
    if (successRate >= 0.8 && averageImprovementScore >= 0.3) return PresetQuality.excellent;
    if (successRate >= 0.6 && averageImprovementScore >= 0.2) return PresetQuality.good;
...
class FavoritePresetStats {
  ...
  factory FavoritePresetStats.fromPresets(List<FavoritePreset> presets) {
    ...
    final avgSuccessRate = presets.map((p) => p.successRate).reduce((a, b) => a + b) / presets.length;
    final avgImprovementScore = presets.map((p) => p.averageImprovementScore).reduce((a, b) => a + b) / presets.length;

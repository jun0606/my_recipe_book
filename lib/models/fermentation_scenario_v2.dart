...
class AlertSettings {
  ...
  factory AlertSettings.fromJson(Map<String, dynamic> json) {
    return AlertSettings(
      ...
      notificationTypes: List<String>.from(json['notificationTypes'] ?? ['push', 'sound']),
...
class RecipeAnalysis {
  ...
  factory RecipeAnalysis.fromJson(Map<String, dynamic> json) {
    return RecipeAnalysis(
      ...
      yeastType: YeastType.values.firstWhere((e) => e.name == json['yeastType']),
...
class FermentationScenarioV2 {
  ...
  List<String> validate() {
    ...
    for (int i = 0; i < stages.length - 1; i++) {
      ...
      final tempDiff = (current.temperature.optimal - next.temperature.optimal).abs();
      if (tempDiff > 20) {
        warnings.add('${current.name}과 ${next.name} 사이의 온도 차이가 큽니다 (${tempDiff.toStringAsFixed(1)}°C).');
      }
    }

    // 4. 장비 호환성 검증
    if (mode == FermentationMode.fermenter && equipment?.fermenterType == null) {

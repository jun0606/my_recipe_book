...
class RecipeHistory {
  ...
  factory RecipeHistory.fromJson(Map<String, dynamic> json) {
    return RecipeHistory(
      ...
      originalIngredients: Map<String, dynamic>.from(json['originalIngredients'] as Map),
      optimizedIngredients: Map<String, dynamic>.from(json['optimizedIngredients'] as Map),
      environmentalConditions: Map<String, dynamic>.from(json['environmentalConditions'] as Map),
      beforeStatus: ProductStatusHistory.fromJson(json['beforeStatus'] as Map<String, dynamic>),
      afterStatus: ProductStatusHistory.fromJson(json['afterStatus'] as Map<String, dynamic>),
  ...
  factory RecipeHistory.create({
    ...
    final improvementScore = _calculateImprovementScore(beforeStatus, afterStatus);
  ...
  static double _calculateImprovementScore(
    ...
    final appearanceImprovement = after.appearanceScore - before.appearanceScore;
...
class ProductStatusHistory {
  ...
  factory ProductStatusHistory.fromProductStatus(dynamic productStatus) {
    return ProductStatusHistory(
      ...
      textureScore: _calculateTextureScore(productStatus.textureProfile?.primary ?? ''),
      flavorScore: _calculateFlavorScore(productStatus.flavorProfile?.primary ?? []),
      appearanceScore: _calculateAppearanceScore(productStatus.appearanceProfile?.crustColor ?? ''),
...
class RecipeHistoryFilter {
  ...
  bool matches(RecipeHistory history) {
    ...
    if (minImprovementScore != null && history.improvementScore < minImprovementScore!) return false;
    if (successfulOnly == true && !history.isSuccessfulOptimization) return false;
...
class RecipeHistoryStats {
  ...
  double get successRate => 
      totalOptimizations > 0 ? successfulOptimizations / totalOptimizations : 0.0;
  ...
  factory RecipeHistoryStats.fromHistories(List<RecipeHistory> histories) {
    ...
    final successful = histories.where((h) => h.isSuccessfulOptimization).length;
    final avgScore = histories.map((h) => h.improvementScore).reduce((a, b) => a + b) / histories.length;
    ...
    for (final history in histories) {
      // 카테고리 분포
      categoryDist[history.recipeCategory] = (categoryDist[history.recipeCategory] ?? 0) + 1;
      ...
      // 환경 조건
      tempSum.add(history.environmentalConditions['temperature']?.toDouble() ?? 26.0);
      humiditySum.add(history.environmentalConditions['humidity']?.toDouble() ?? 60.0);
      altitudeSum.add(history.environmentalConditions['altitude']?.toDouble() ?? 0.0);
    }
    ...
    final topImprovements = sortedImprovements
        .take(3)
        .map((e) => e.key)
        .toList();
    ...
    final envPrefs = <String, double>{
      'temperature': tempSum.isNotEmpty ? tempSum.reduce((a, b) => a + b) / tempSum.length : 26.0,
      'humidity': humiditySum.isNotEmpty ? humiditySum.reduce((a, b) => a + b) / humiditySum.length : 60.0,
      'altitude': altitudeSum.isNotEmpty ? altitudeSum.reduce((a, b) => a + b) / altitudeSum.length : 0.0,
    };

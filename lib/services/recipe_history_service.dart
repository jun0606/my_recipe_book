...
class RecipeHistoryService {
  ...
  static RecipeHistoryService get instance => _instance ??= RecipeHistoryService._();
  ...
  /// 필터링된 히스토리 조회
  Future<List<RecipeHistory>> getFilteredHistories(RecipeHistoryFilter filter) async {
    await loadHistories();
  ...
  Future<List<RecipeHistory>> getHistoriesByRecipe(String recipeTitle) async {
    await loadHistories();
    return _histories.where((history) => history.recipeTitle == recipeTitle).toList();
  }
  ...
  Future<List<RecipeHistory>> getSuccessfulOptimizations() async {
    await loadHistories();
    return _histories.where((history) => history.isSuccessfulOptimization).toList();
  }
  ...
  /// 특정 기간의 통계
  Future<RecipeHistoryStats> getStatsForPeriod(DateTime startDate, DateTime endDate) async {
  ...
  Future<void> importHistories(String jsonString) async {
    try {
      ...
      final newHistories = importedHistories.where((h) => !existingIds.contains(h.id)).toList();
  ...
  Future<Map<String, dynamic>> getUserPreferences() async {
    ...
    return {
      'favoriteCategories': stats.categoryDistribution.entries
          .toList()
  ...
  Future<Map<String, double>> getRecommendedEnvironment({
    ...
    if (category != null) {
      relevantHistories = relevantHistories.where((h) => h.recipeCategory == category);
    }
    ...
    final tempSum = histories.map((h) => h.environmentalConditions['temperature']?.toDouble() ?? 26.0).reduce((a, b) => a + b);
    final humiditySum = histories.map((h) => h.environmentalConditions['humidity']?.toDouble() ?? 60.0).reduce((a, b) => a + b);
    final altitudeSum = histories.map((h) => h.environmentalConditions['altitude']?.toDouble() ?? 0.0).reduce((a, b) => a + b);
  ...
  Future<Map<String, int>> getMonthlyUsageStats() async {
    ...
    for (final history in _histories) {
      final monthKey = '${history.timestamp.year}-${history.timestamp.month.toString().padLeft(2, '0')}';
  ...
  Future<Map<String, List<double>>> getImprovementTrends() async {
    ...
    for (final history in _histories.reversed) { // 시간순으로 정렬
      final textureImprovement = history.afterStatus.textureScore - history.beforeStatus.textureScore;
      final flavorImprovement = history.afterStatus.flavorScore - history.beforeStatus.flavorScore;
      final appearanceImprovement = history.afterStatus.appearanceScore - history.beforeStatus.appearanceScore;

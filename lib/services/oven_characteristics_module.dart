...
class OvenCharacteristicsModule extends AdjustmentModule {
  ...
  @override
  List<BakingType> get supportedTypes =>
      [BakingType.bread, BakingType.cake, BakingType.cookie];
  ...
  Map<String, double> calculate(Map<String, dynamic> inputs) {
    ...
    // 레시피 특성에 따른 추가 보정
    final recipeAdjustments =
        _calculateRecipeAdjustments(recipeTemp, recipeTime);
  ...
  Map<String, double> _calculateOvenTypeAdjustments(String ovenType,
      bool useFan, String fanSpeed, String steamAmount, String combiMode) {
    Map<String, double> adjustments = {};
  ...
  Map<String, double> _calculateRecipeAdjustments(
      double recipeTemp, double recipeTime) {
    final adjustments = <String, double>{};
  ...
  /// 레시피 특성 기반 설명 추가
  void _addRecipeBasedExplanations(
      List<String> explanations, double recipeTemp, double recipeTime) {
    // 레시피 타입 자동 인식 및 설명
    ...
    if (recipeTemp > 220.0) {
      explanations
          .add('🔥 매우 높은 온도(${recipeTemp.toInt()}°C): 빠른 갈변과 바삭한 식감을 위한 설정');
  ...
  /// 레시피별 구체적인 팁 제공
  void _addRecipeSpecificTips(
      List<String> explanations, double temp, double time) {
    if (temp > 220.0) {

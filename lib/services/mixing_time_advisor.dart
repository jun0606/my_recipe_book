...
class MixingTimeAdvisor {
  ...
  static MixingTimeResult calculateOptimalMixingTime({
    ...
    try {
      // 1. 기본 믹싱 특성 분석
      final mixingCharacteristics =
          _analyzeMixingCharacteristics(doughState, ingredients, recipeTitle);
      ...
      // 3. 단계별 믹싱 시간 계산
      final lowSpeedTime =
          _calculateLowSpeedTime(mixingCharacteristics, mixerEfficiency);
      final mediumSpeedTime =
          _calculateMediumSpeedTime(mixingCharacteristics, mixerEfficiency);
      final highSpeedTime =
          _calculateHighSpeedTime(mixingCharacteristics, mixerEfficiency);
      ...
      final completionCriteria =
          _generateCompletionCriteria(doughState, mixingCharacteristics);
  ...
  static Map<String, dynamic> _analyzeMixingCharacteristics(
    ...
    // 글루텐 강도에 따른 믹싱 난이도
    final glutenDifficulty = doughState.glutenStrengthIndex > 12.0
        ? 'high'
        : doughState.glutenStrengthIndex > 8.0
            ? 'medium'
            : 'low';

    // 수분율에 따른 믹싱 특성
    final hydrationEffect = doughState.hydrationLevel > 0.75
        ? 'sticky'
        : doughState.hydrationLevel > 0.65
            ? 'normal'
            : 'stiff';

    // 점성에 따른 저항력
    final viscosityResistance = doughState.viscosityIndex > 2.0
        ? 'high'
        : doughState.viscosityIndex > 1.0
            ? 'medium'
            : 'low';
    ...
    return {
      ...
      'fatEffect': fatEffect,
      'overallComplexity': _calculateOverallComplexity(
          glutenDifficulty, hydrationEffect, viscosityResistance),
    };
  }

  /// 믹서 효율성 계산
  static Map<String, double> _calculateMixerEfficiency(
      String mixerType, double doughWeight) {
  ...
  static int _calculateLowSpeedTime(
    ...
    final adjustedTime = (baseTime /
            mixerEfficiency['lowSpeedFactor']! /
            mixerEfficiency['weightCapacity']!)
        .round();
  ...
  static int _calculateMediumSpeedTime(
    ...
    // 믹서 효율성 적용
    final adjustedTime = (baseTime /
            mixerEfficiency['mediumSpeedFactor']! /
            mixerEfficiency['weightCapacity']!)
        .round();
  ...
  static int _calculateHighSpeedTime(
    ...
    // 믹서 효율성 적용
    final adjustedTime = (baseTime /
            mixerEfficiency['highSpeedFactor']! /
            mixerEfficiency['weightCapacity']!)
        .round();
  ...
  /// 전체 복잡도 계산
  static String _calculateOverallComplexity(String glutenDifficulty,
      String hydrationEffect, String viscosityResistance) {
    int complexityScore = 0;

    if (glutenDifficulty == 'high')
      complexityScore += 2;
    else if (glutenDifficulty == 'medium') complexityScore += 1;

    if (hydrationEffect == 'sticky')
      complexityScore += 2;
...
class MixingTimeResult {
  ...
  factory MixingTimeResult.fromJson(Map<String, dynamic> json) {
    return MixingTimeResult(
      ...
      glutenDevelopmentPrediction:
          json['glutenDevelopmentPrediction'] ?? "예측 불가",
      completionCriteria:
          List<String>.from(json['completionCriteria'] ?? ["기본 완료 기준 적용"]),
      mixingCharacteristics:
          Map<String, dynamic>.from(json['mixingCharacteristics'] ?? {}),
      calculationTimestamp: DateTime.parse(
          json['calculationTimestamp'] ?? DateTime.now().toIso8601String()),
    );
  }
  ...
  String get complexityText {
    final complexity =
        mixingCharacteristics['overallComplexity'] as String? ?? 'medium';
    switch (complexity) {
      case 'high':
        return '높음 (주의 필요)';
      case 'medium':

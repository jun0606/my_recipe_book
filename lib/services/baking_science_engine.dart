...
class BakingScienceEngine {
  ...
  static MaillardReactionResult calculateMaillardReaction({
    ...
    double temperatureEffect =
        math.pow((temperature - maillardThreshold) / 60.0, 1.5).toDouble();
    ...
    double moistureEffect =
        moisture < 0.3 ? 1.0 : (1.0 - (moisture - 0.3) * 0.5);
    moistureEffect = math.max(moistureEffect, 0.3);

    // 전체 반응 강도 계산
    double intensity = temperatureEffect *
        timeEffect *
        sugarEffect *
        proteinEffect *
        moistureEffect;
    ...
    // 풍미 화합물 예측
    List<String> flavorCompounds =
        _predictFlavorCompounds(intensity, sugarContent, proteinContent);
  ...
  static GlutenStrengthResult predictGlutenStrength({
    ...
    double totalStrength =
        baseStrength * hydrationEffect * kneadingEffect * saltEffect;
  ...
  static FermentationOptimizationResult optimizeFermentationTime({
    ...
    // 최적 발효 시간 계산
    double optimalTime = baseFermentationMinutes /
        (temperatureEffect * yeastEffect * sugarEffect) *
        saltEffect *
        sizeEffect;
    ...
    return FermentationOptimizationResult(
      ...
      optimalTemperature:
          _calculateOptimalFermentationTemperature(yeastPercentage),
      expectedVolumeIncrease:
          _calculateExpectedVolumeIncrease(yeastPercentage, sugarContent),
    );
  }
  ...
  static OvenSpringResult predictOvenSpring({
    ...
    // 전체 오븐 스프링 계산
    double ovenSpringFactor = glutenEffect *
        fermentationEffect *
        temperatureEffect *
        steamEffect *
        tensionEffect;
    ...
    return OvenSpringResult(
      ...
      crustFormationTime:
          _calculateCrustFormationTime(ovenTemperature, steamPresence),
      optimalBakingStrategy:
          _recommendBakingStrategy(ovenSpringFactor, steamPresence),
    );
  }
  ...
  /// 풍미 화합물 예측
  static List<String> _predictFlavorCompounds(
      double intensity, double sugarContent, double proteinContent) {
  ...
  /// 글루텐 강도에 따른 식감 예측
  static String _predictTextureFromGluten(
      GlutenStrengthLevel level, double hydration) {
    switch (level) {
  ...
  /// 최적 발효 온도 계산
  static double _calculateOptimalFermentationTemperature(
  ...
  /// 예상 부피 증가율 계산
  static double _calculateExpectedVolumeIncrease(
      double yeastPercentage, double sugarContent) {
  ...
  /// 크러스트 형성 시간 계산
  static double _calculateCrustFormationTime(
      double ovenTemperature, double steamPresence) {
  ...
  /// 베이킹 전략 추천
  static String _recommendBakingStrategy(
      double ovenSpringFactor, double steamPresence) {

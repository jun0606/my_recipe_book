...
class DivisionCalculationEngine {
  ...
  static DivisionResult calculateByCount({
    ...
    if (scaleFactor > 10) {
      throw ArgumentError(
        '너무 많은 양입니다 (${scaleFactor.toStringAsFixed(1)}배). '
  ...
  static List<OptimalDivisionSuggestion> suggestOptimalDivisions({
    ...
    for (int count = minCount; count <= maxCount; count++) {
      ...
      final convenienceScore = _calculateConvenienceScore(count, originalRecipe.originalYield);
  ...
  static double _calculateWasteScore(DivisionResult result) {
    ...
    final totalWasteWeight = result.remainder.values.fold(0.0, (sum, amount) => sum + amount);
  ...
  static ValidationResult validateCalculation({
    ...
    // 과도한 배수 경고
    if (scaleFactor > 5) {
      warnings.add('${scaleFactor.toStringAsFixed(1)}배는 꽤 많은 양입니다. 오븐 용량을 확인해주세요.');
    }
    
    if (scaleFactor > 10) {
      errors.add('${scaleFactor.toStringAsFixed(1)}배는 너무 많습니다. 10배 이하로 조정해주세요.');
    }
    
    // 너무 작은 양 경고
    if (scaleFactor < 0.5) {
      warnings.add('${scaleFactor.toStringAsFixed(1)}배는 꽤 적은 양입니다. 재료 측정이 어려울 수 있어요.');
    }
    
    // 복잡한 비율 경고
    if (scaleFactor != scaleFactor.round() && (scaleFactor * 2) != (scaleFactor * 2).round()) {

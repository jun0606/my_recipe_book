...
class FavoritePresetService {
  ...
  static FavoritePresetService get instance => _instance ??= FavoritePresetService._();
  ...
  /// 필터링된 즐겨찾기 조회
  Future<List<FavoritePreset>> getFilteredFavorites(FavoritePresetFilter filter) async {
    await loadFavorites();
  ...
  Future<List<FavoritePreset>> getFavoritesByCategory(String category) async {
    await loadFavorites();
    return _favorites.where((favorite) => favorite.category == category).toList();
  }
  ...
  /// 최근 사용한 즐겨찾기 조회
  Future<List<FavoritePreset>> getRecentlyUsedFavorites({int limit = 10}) async {
    await loadFavorites();
  ...
  void _sortFavorites(FavoritePresetSortOption sortOption) {
    switch (sortOption) {
      case FavoritePresetSortOption.popularity:
        _favorites.sort((a, b) => b.popularityScore.compareTo(a.popularityScore));
        break;
      ...
      case FavoritePresetSortOption.improvementScore:
        _favorites.sort((a, b) => b.averageImprovementScore.compareTo(a.averageImprovementScore));
        break;
  ...
  /// 정렬된 즐겨찾기 조회
  Future<List<FavoritePreset>> getSortedFavorites(FavoritePresetSortOption sortOption) async {
    await loadFavorites();
    final sorted = List<FavoritePreset>.from(_favorites);
    ...
    switch (sortOption) {
      ...
      case FavoritePresetSortOption.successRate:
        ...
        break;
      case FavoritePresetSortOption.improvementScore:
        sorted.sort((a, b) => b.averageImprovementScore.compareTo(a.averageImprovementScore));
        break;
      case FavoritePresetSortOption.usageCount:
  ...
  Future<List<Map<String, dynamic>>> getAutoFavoriteRecommendations() async {
    try {
      final histories = await RecipeHistoryService.instance.getSuccessfulOptimizations();
      ...
      for (final entry in categorySuccess.entries) {
        ...
        if (categoryHistories.length >= 3) { // 최소 3번 이상 성공
          // 가장 일관된 환경 조건 찾기
          final avgTemp = categoryHistories
              .map((h) => h.environmentalConditions['temperature']?.toDouble() ?? 26.0)
              .reduce((a, b) => a + b) / categoryHistories.length;
  ...
  double _calculateConfidence(List<RecipeHistory> histories) {
    ...
    final avgImprovement = histories
        .map((h) => h.improvementScore)
        .reduce((a, b) => a + b) / histories.length;
    
    final consistency = _calculateConsistency(histories);
    final sampleSize = (histories.length / 10.0).clamp(0.0, 1.0);
    
    return (avgImprovement * 0.4 + consistency * 0.4 + sampleSize * 0.2).clamp(0.0, 1.0);
  }
  ...
  double _calculateConsistency(List<RecipeHistory> histories) {
    ...
    final variance = improvements
        ...
        .reduce((a, b) => a + b) / improvements.length;
  ...
  Future<void> importFavorites(String jsonString) async {
    try {
      ...
      final newFavorites = importedFavorites.where((f) => !existingIds.contains(f.id)).toList();
  ...
  Future<bool> isFavorite(String presetId) async {
    await loadFavorites();
    return _favorites.any((f) => f.originalPresetId == presetId || f.id == presetId);
  }
  ...
  Future<void> cleanupUnusedFavorites({int daysThreshold = 30}) async {
    ...
    _favorites.removeWhere((favorite) => 
        ...
        favorite.lastUsedAt.isBefore(cutoffDate));
  ...
  Future<void> duplicateFavorite(String favoriteId, {String? newName}) async {
    ...
    final duplicate = FavoritePreset.createCustom(
      ...
      environmentalConditions: Map<String, dynamic>.from(original.environmentalConditions),

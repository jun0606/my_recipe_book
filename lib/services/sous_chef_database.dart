...
class SousChefDatabase {
  ...
  static Box<SousChefPreset> get presetsBox {
    if (_presetsBox == null) {
      throw Exception('SousChefDatabase not initialized. Call initialize() first.');
    }
    return _presetsBox!;
  }
  ...
  /// 프리셋 성공률 업데이트
  static Future<void> updatePresetSuccessRate(String presetId, double successRate) async {
    final preset = presetsBox.get(presetId);
    if (preset != null) {
      final updatedPreset = SousChefPreset(
        id: preset.id,

...
class EnvironmentCorrectionEngine {
  ...
  static double calculateFermentationSpeedCorrection(double actualTemperature) {
    ...
    double exponent = (actualTemperature - baseTemperature) / temperatureSensitivity;
  ...
  static EnvironmentCorrectionResult calculateIntegratedCorrection(
    ...
    // 개별 보정 계수 계산
    final temperatureCorrection = calculateFermentationSpeedCorrection(conditions.temperature);
    ...
    // 통합 보정 계수 (곱셈 적용)
    final integratedCorrection = temperatureCorrection * humidityCorrection * altitudeCorrection;
  ...
  static int applyFermentationTimeCorrection(
    ...
    double correctedTime = baseFermentationTime / correction.temperatureCorrection;
  ...
  static int applyBakingTemperatureCorrection(
    ...
    if (correction.conditions.altitude > 500) {
      correctedTemp += (correction.conditions.altitude / 1000) * 15; // 1000m당 15°C 증가
    }
    
    // 습도가 높을수록 온도를 약간 높여야 함
    if (correction.conditions.humidity > 70) {
      correctedTemp += (correction.conditions.humidity - 70) * 0.2; // 1%당 0.2°C 증가
    }
  ...
  static Map<String, double> applyIngredientCorrection(
    ...
    for (final entry in baseIngredients.entries) {
      ...
      if (_isFlour(ingredientName)) {
        ...
        correctedAmount /= correction.altitudeCorrection;
        correctedAmount /= math.sqrt(correction.temperatureCorrection); // 제곱근 적용으로 완화
      } else if (_isSugar(ingredientName)) {
        // 설탕: 습도 영향 최소화
        correctedAmount *= math.sqrt(correction.humidityCorrection);
      } else if (_isFat(ingredientName)) {
        // 지방: 온도 영향 고려
        correctedAmount *= (2.0 - correction.temperatureCorrection); // 온도가 높으면 약간 감소
      }
  ...
  static Future<EnvironmentalConditions> getEnvironmentalDataFromSensors() async {
  ...
  static bool _isFlour(String name) {
    ...
    return flourKeywords.any((keyword) => name.toLowerCase().contains(keyword.toLowerCase()));
  }
  
  static bool _isLiquid(String name) {
    final liquidKeywords = ['물', 'water', '우유', 'milk', '크림', 'cream'];
    return liquidKeywords.any((keyword) => name.toLowerCase().contains(keyword.toLowerCase()));
  }
  
  static bool _isLeavening(String name) {
    final leavenKeywords = ['이스트', 'yeast', '베이킹파우더', 'baking powder', '베이킹소다', 'baking soda'];
    return leavenKeywords.any((keyword) => name.toLowerCase().contains(keyword.toLowerCase()));
  }
  
  static bool _isSugar(String name) {
    final sugarKeywords = ['설탕', 'sugar', '꿀', 'honey', '시럽', 'syrup'];
    return sugarKeywords.any((keyword) => name.toLowerCase().contains(keyword.toLowerCase()));
  }
  
  static bool _isFat(String name) {
    final fatKeywords = ['버터', 'butter', '기름', 'oil', '마가린', 'margarine'];
    return fatKeywords.any((keyword) => name.toLowerCase().contains(keyword.toLowerCase()));
  }
}

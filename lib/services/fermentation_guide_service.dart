...
class FermentationGuideService implements IFermentationGuideService {
  ...
  Future<EnvironmentalAnalysis> analyzeEnvironment({
    ...
    final defaults = _fermentationDefaults[fermentationType] ??
        _fermentationDefaults['room_temperature']!;
  ...
  Future<RecipeAnalysis> analyzeRecipe({
    ...
    final optimalYeastRatio =
        _calculateOptimalYeastRatio(fermentationType, totalFlour);
    final optimalSugarRatio =
        _calculateOptimalSugarRatio(fermentationType, totalFlour);
    ...
    final overallAssessment =
        _generateOverallAssessment(yeastStatus, sugarStatus);
  ...
  Future<List<FermentationRecommendation>> generateRecommendations({
    ...
    recommendations.add(FermentationRecommendation(
      ...
      title: '발효 시간 모니터링',
      description:
          '예상 발효 시간: ${_formatDuration(Duration(minutes: recipeAnalysis.estimatedFermentationTimeMinutes))}',
      actionItems: [
        '30분마다 발효 상태를 확인하세요',
  ...
  List<String> _generateCustomInstructions(
    ...
    if (environmentalAnalysis.temperatureStatus != 'optimal') {
      instructions.add(
          '현재 온도(${environmentalAnalysis.currentTemperature.toStringAsFixed(1)}°C)가 '
    ...
    if (recipeAnalysis.yeastStatus != 'optimal') {
      instructions.add(
          '이스트 양이 ${recipeAnalysis.yeastStatus == 'insufficient' ? '부족' : '과다'}합니다. '
  ...
  /// 발효 경고사항 생성
  List<FermentationWarning> _generateFermentationWarnings(
      String fermentationType) {
  ...
  double _calculateOptimalYeastRatio(
      String fermentationType, double flourAmount) {
    switch (fermentationType) {
      case 'room_temperature':
  ...
  double _calculateOptimalSugarRatio(
      String fermentationType, double flourAmount) {
  ...
  List<String> _generateOptimizationTips({
    ...
    if (yeastStatus == 'insufficient') {
      tips.add(
          '이스트를 ${(optimalYeastRatio - yeastAmount).toStringAsFixed(1)}g 더 추가하세요.');
    } else if (yeastStatus == 'excessive') {
      tips.add(
          '이스트가 ${(yeastAmount - optimalYeastRatio).toStringAsFixed(1)}g 과다합니다. 다음에는 줄여보세요.');
    }

    if (sugarStatus == 'insufficient') {
      tips.add(
          '설탕을 ${(optimalSugarRatio - sugarAmount).toStringAsFixed(1)}g 더 추가하면 발효가 더 활발해집니다.');
  ...
  Duration _estimateFermentationTime({
    ...
    final adjustedMinutes =
        (baseTime.inMinutes * yeastFactor * tempFactor).round();

...
class ScalingEngine {
  ...
  Future<ScalingResult> scaleRecipe({
    ...
    try {
      ...
      calculations['originalTotalWeight'] = _calculateTotalWeight(recipe.ingredients);
      calculations['scaledTotalWeight'] = _calculateTotalWeight(scaledIngredients);

      // 경고 생성
      if (scaleFactor > 5.0) {
        warnings.add(ValidationWarning(
          level: ValidationLevel.warning,
          message: '스케일링 비율이 매우 큽니다 (${scaleFactor.toStringAsFixed(1)}배). 결과를 확인해주세요.',
          category: 'scaling',
        ));
      } else if (scaleFactor < 0.2) {
        warnings.add(ValidationWarning(
          level: ValidationLevel.warning,
          message: '스케일링 비율이 매우 작습니다 (${scaleFactor.toStringAsFixed(1)}배). 정확도가 떨어질 수 있습니다.',
          category: 'scaling',
        ));
      }

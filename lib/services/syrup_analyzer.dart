...
class SyrupAnalyzer {
  ...
  static SyrupAnalysisResult analyzeSyrupEffects({
    ...
    try {
      ...
      // 4. 화학적 작용 분석
      final chemicalEffects =
          _analyzeChemicalEffects(syrupClassification, syrupPercentage);

      // 5. 물리적 작용 분석 (레올로지)
      final physicalEffects =
          _analyzePhysicalEffects(syrupClassification, syrupPercentage);

      // 6. 미생물학적 영향 분석 (발효)
      final fermentationEffects =
          _analyzeFermentationEffects(syrupClassification, syrupPercentage);

      // 7. 굽기 특성 변화 분석
      final bakingEffects =
          _analyzeBakingEffects(syrupClassification, syrupPercentage);

      // 8. 저장성 및 품질 영향
      final storageEffects =
          _analyzeStorageEffects(syrupClassification, syrupPercentage);
  ...
  /// 시럽류 재료 찾기 (확장된 키워드)
  static List<Map<String, dynamic>> _findSyrupIngredients(
    ...
    return ingredients.where((ingredient) {
      ...
      final syrupKeywords = [
        ...
        'galactooligosaccharide', 'xylooligosaccharide', 'fos', 'imo', 'gos',
        'xos',
  ...
  /// 시럽 타입별 분류
  static Map<String, dynamic> _classifySyrupTypes(
      List<Map<String, dynamic>> syrupIngredients) {
    ...
    for (final syrup in syrupIngredients) {
      ...
      // 올리고당류 (환원당 낮음)
      else if (name.contains('올리고당') ||
          name.contains('oligosaccharide') ||
          name.contains('fos') ||
          name.contains('imo') ||
          name.contains('gos') ||
          name.contains('xos')) {
    ...
    return {
      ...
      'dominantType': _determineDominantSyrupType(
          cornSyrupWeight, oligosaccharideWeight, otherSyrupWeight),
    };
  }

  /// 주요 시럽 타입 결정
  static String _determineDominantSyrupType(
      double cornSyrup, double oligo, double other) {
  ...
  /// 총 시럽 무게 계산
  static double _calculateTotalSyrupWeight(
      List<Map<String, dynamic>> syrupIngredients) {
  ...
  static Map<String, dynamic> _analyzeChemicalEffects(
      Map<String, dynamic> classification, double syrupPercentage) {
    ...
    if (reducingSugarIndex >= 7.0)
      browningSpeed = 'fast';
    else if (reducingSugarIndex >= 4.0)
      browningSpeed = 'moderate';
    else
      browningSpeed = 'slow';

    // 마이야르 반응 강도
    String maillardIntensity = 'medium';
    if (cornSyrupWeight > 0 && syrupPercentage >= 5.0)
      maillardIntensity = 'high';
  ...
  static Map<String, dynamic> _analyzePhysicalEffects(
    ...
    if (syrupPercentage >= 8.0)
      glutenHydrationEffect = 'reduced';
    else if (syrupPercentage >= 5.0) glutenHydrationEffect = 'slightly_reduced';

    // 반죽 질감 변화
    String doughTexture = 'normal';
    if (syrupPercentage >= 10.0)
      doughTexture = 'very_soft';
    else if (syrupPercentage >= 6.0)
      doughTexture = 'soft';
    else if (syrupPercentage >= 3.0) doughTexture = 'slightly_soft';

    // 가스 보유력 영향
    String gasRetentionEffect = 'normal';
    if (syrupPercentage >= 12.0)
      gasRetentionEffect = 'reduced';
    else if (syrupPercentage >= 8.0)
      gasRetentionEffect = 'slightly_reduced';
    else
      gasRetentionEffect = 'improved';
  ...
  static Map<String, dynamic> _analyzeBakingEffects(
      Map<String, dynamic> classification, double syrupPercentage) {
    final chemicalEffects =
        _analyzeChemicalEffects(classification, syrupPercentage);
  ...
  static List<String> _generateRecommendations(
    ...
    // 발효 조정
    final fermentationEffects =
        _analyzeFermentationEffects(classification, syrupPercentage);
    final yeastAdjustment =
        fermentationEffects['yeastAdjustmentNeeded'] as double;
    ...
    // 온도 조정
    final bakingEffects =
        _analyzeBakingEffects(classification, syrupPercentage);
    final tempAdjustment = bakingEffects['temperatureAdjustment'] as int;
    if (tempAdjustment != 0) {
      recommendations.add(
          '오븐 온도 ${tempAdjustment.abs()}°C ${tempAdjustment < 0 ? '낮춤' : '높임'} 권장');
    }
    ...
    // 발효 시간 조정
    final fermentationSpeed =
        fermentationEffects['fermentationSpeed'] as String;
...
class SyrupAnalysisResult {
  ...
  String get syrupTypeText {
    final dominantType =
        syrupClassification['dominantType'] as String? ?? 'none';
    switch (dominantType) {
      case 'corn_syrup':
        return '물엿 주도형';
  ...
  String get fermentationEffectText {
    final speed =
        fermentationEffects['fermentationSpeed'] as String? ?? 'normal';
    switch (speed) {
      case 'very_slow':
        return '매우 느림';
      case 'slow':
  ...
  String get moistureEffectText {
    final retention =
        storageEffects['moistureRetention'] as String? ?? 'normal';
    switch (retention) {
      case 'excellent':
        return '우수';
      case 'good':

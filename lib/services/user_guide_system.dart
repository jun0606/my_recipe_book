...
class UserGuide {
  ...
  /// 전체 진행률 (0.0 ~ 1.0)
  double get progress => steps.isEmpty ? 0.0 : completedStepsCount / steps.length;
...
class UserGuideSystem {
  ...
  Future<UserGuide> createFermenterSetupGuide({
    ...
    for (int i = 0; i < stages.length; i++) {
      ...
      steps.add(GuideStep(
        ...
        instructions: instructions.expand((inst) => [
          inst.description,
          ...inst.steps,
        ]).toList(),
  ...
  Future<UserGuide> createFermentationProgressGuide({
    ...
    final guideId = 'fermentation_progress_${DateTime.now().millisecondsSinceEpoch}';
    final steps = <GuideStep>[];
  ...
  Future<UserGuide> createTroubleshootingGuide({
    ...
    final troubleshootingGuide = FermenterGuideManager.getTroubleshootingGuide();
  ...
  /// 자동 진행 처리
  Future<void> _handleAutoProgress(UserGuide guide, int completedStepIndex) async {
    // 다음 단계가 있는지 확인
  ...
  Future<void> _loadSettings() async {
    ...
    if (settingsJson != null) {
      ...
      _smartNotificationsEnabled = settings['smartNotificationsEnabled'] ?? true;
  ...
  Future<void> _saveActiveGuides() async {
    final prefs = await SharedPreferences.getInstance();
    final guidesList = _activeGuides.values.map((guide) => guide.toJson()).toList();

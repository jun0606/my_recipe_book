...
class EnvironmentalAdjustmentEngine {
  ...
  Future<Map<String, dynamic>> calculateOptimalConditions(
    ...
    try {
      ...
      final ingredients = sousChefData['ingredients'] as List<Map<String, dynamic>>;
      ...
      for (final ingredient in ingredients) {
        ...
        if (_isFlour(name)) {
          ...
          optimizedAmount *= (1.0 + humidityFactor * 0.01 + altitudeFactor * 0.02);
    ...
    try {
      ...
      for (final ingredient in recipe.ingredients) {
        ...
        if (_isFlour(name)) {
          ...
          adjustedAmount *= (1.0 + humidityFactor * 0.01 + altitudeFactor * 0.02);
        } else if (_isLeavening(name)) {
  ...
  bool _isFlour(String name) {
    return name.contains('밀가루') || name.contains('flour') || name.contains('가루');
  }

  /// 액체 재료 판별
  bool _isLiquid(String name) {
    return name.contains('물') || name.contains('우유') || name.contains('기름') ||
           name.contains('water') || name.contains('milk') || name.contains('oil');
  }

  /// 팽창제 재료 판별
  bool _isLeavening(String name) {
    return name.contains('이스트') || name.contains('베이킹파우더') || name.contains('소다') ||
           name.contains('yeast') || name.contains('baking powder') || name.contains('soda');
  }

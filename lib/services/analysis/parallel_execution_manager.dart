...
class ParallelExecutionManager {
  ...
  /// Isolate 내에서 태스크 실행
  static Future<Map<String, dynamic>> _executeTaskInIsolate(TaskMessage message) async {
  ...
  Future<Map<String, Map<String, dynamic>>> executeInParallel(
    ...
    try {
      // 모듈들을 태스크로 변환
      final tasks = modules.map((module) => PendingTask(
        id: _generateTaskId(),
        module: module,
        request: request,
      ...
      while (_taskQueue.isNotEmpty || _activeTasks.isNotEmpty) {
        ...
        await Future.delayed(Duration(milliseconds: 10)); // 짧은 대기
      }
  ...
  bool _canStartNewTask() {
    ...
    if (_enableResourceMonitoring) {
      ...
      if (resourceUsage.memoryUsagePercent > 80 || resourceUsage.cpuUsagePercent > 90) {
        return false;
      }
    }
  ...
  /// Isolate에서 실행
  Future<void> _executeInIsolate(PendingTask task, TaskExecution execution) async {
  ...
  /// 메인 스레드에서 실행
  Future<void> _executeInMainThread(PendingTask task, TaskExecution execution) async {
    try {
      final result = await task.module.analyze(task.request).timeout(_taskTimeout);
      execution.result = result;
      execution.endTime = DateTime.now();
      execution.success = true;
    } catch (e) {
  ...
  /// 태스크 우선순위 계산
  int _calculateTaskPriority(BaseAnalysisModule module, AnalysisRequest request) {
  ...
  ParallelExecutionStatus getExecutionStatus() {
    return ParallelExecutionStatus(
      ...
      resourceUsage: _enableResourceMonitoring ? _resourceMonitor.getCurrentUsage() : null,
...
class TaskResult {
  ...
  TaskResult.success(this.taskId, this.result) : isSuccess = true, error = null;
  TaskResult.error(this.taskId, this.error) : isSuccess = false, result = null;
}
  ...
  ExecutionStatisticsSnapshot getSnapshot() {
    return ExecutionStatisticsSnapshot(
      ...
      averageExecutionTime: _executionTimes.isNotEmpty
          ? Duration(milliseconds: _executionTimes.map((d) => d.inMilliseconds).reduce((a, b) => a + b) ~/ _executionTimes.length)
          : Duration.zero,
      averageTaskDuration: _taskDurations.isNotEmpty
          ? Duration(milliseconds: _taskDurations.map((d) => d.inMilliseconds).reduce((a, b) => a + b) ~/ _taskDurations.length)
          : Duration.zero,
      successRate: _totalExecutions > 0 ? _successfulExecutions / _totalExecutions : 0.0,
      taskSuccessRate: _totalTasksExecuted > 0 ? _totalTasksSucceeded / _totalTasksExecuted : 0.0,
    );
  }
}

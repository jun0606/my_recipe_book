...
class ErrorRecoveryManager {
  ...
  Future<Map<String, dynamic>> _executeWithStrategies(
    ...
    for (final strategy in strategies) {
      try {
        ...
        _notifyListeners(ErrorRecoveryEvent.strategyApplied(execution, strategy.name));
        
        final result = await strategy.execute(module, request, execution);
        
        _notifyListeners(ErrorRecoveryEvent.strategySucceeded(execution, strategy.name));
        return result;
        
      } catch (e) {
        execution.strategyErrors[strategy.name] = e;
        _notifyListeners(ErrorRecoveryEvent.strategyFailed(execution, strategy.name, e));
  ...
  List<RecoveryStrategy> _selectStrategies(
    ...
    if (errorHistory != null) {
      ...
      } else if (recentErrors.any((e) => e.errorType == ErrorType.resourceExhaustion)) {
        // 리소스 부족 시 격리 전략
  ...
  bool _isCircuitBreakerOpen(String moduleName) {
    ...
    if (state.state == CircuitState.open) {
      // 타임아웃 확인
      if (DateTime.now().difference(state.lastFailureTime) > _circuitBreakerTimeout) {
        // 반열림 상태로 전환
  ...
  /// 오류 기록
  void _recordError(String moduleName, Object error, ErrorRecoveryExecution execution) {
    // 오류 이력 업데이트
    ...
    // 회로 차단기 상태 업데이트
    final state = _circuitBreakers.putIfAbsent(moduleName, () => CircuitBreakerState(moduleName));
  ...
  Map<String, ErrorStatistics> getAllErrorStatistics() {
    return _errorHistory.map((name, history) => MapEntry(name, history.getStatistics()));
  }
      ...
      try {
        if (attempt > 1) {
          final delay = Duration(
            milliseconds: (baseDelay.inMilliseconds * math.pow(backoffMultiplier, attempt - 2)).round(),
          );
...
class FallbackStrategy implements RecoveryStrategy {
  ...
  Map<String, dynamic> _generateFallbackResult(BaseAnalysisModule module, AnalysisRequest request) {
    return {
      'fallback': true,
...
class PartialRecoveryStrategy implements RecoveryStrategy {
  ...
  Future<Map<String, dynamic>> _attemptPartialAnalysis(BaseAnalysisModule module, AnalysisRequest request) async {
    // 간단한 분석만 수행 (타임아웃 짧게 설정)
    return await module.analyze(request).timeout(Duration(seconds: 5));
  }

  Map<String, dynamic> _generateMinimalResult(BaseAnalysisModule module, AnalysisRequest request) {
    return {
...
class IsolationStrategy implements RecoveryStrategy {
  ...
  Future<Map<String, dynamic>> _executeInIsolation(BaseAnalysisModule module, AnalysisRequest request) async {
...
class ErrorHistory {
  ...
  ErrorStatistics getStatistics() {
    ...
    final last24Hours = _errors.where((e) => now.difference(e.timestamp).inHours < 24).length;
    final lastHour = _errors.where((e) => now.difference(e.timestamp).inMinutes < 60).length;
...
class ErrorStatistics {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'error_type_counts': errorTypeCounts.map((k, v) => MapEntry(k.toString(), v)),
...
abstract class ErrorRecoveryEvent {
  ...
  static ErrorRecoveryEvent executionStarted(ErrorRecoveryExecution execution) =>
      ExecutionStartedEvent(execution);
  static ErrorRecoveryEvent executionSucceeded(ErrorRecoveryExecution execution) =>
  ...
  static ErrorRecoveryEvent strategyApplied(ErrorRecoveryExecution execution, String strategyName) =>
      StrategyAppliedEvent(execution, strategyName);
  static ErrorRecoveryEvent strategySucceeded(ErrorRecoveryExecution execution, String strategyName) =>
      StrategySucceededEvent(execution, strategyName);
  static ErrorRecoveryEvent strategyFailed(ErrorRecoveryExecution execution, String strategyName, Object error) =>

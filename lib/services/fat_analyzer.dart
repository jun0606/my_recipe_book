...
class FatAnalyzer {
  ...
  static FatAnalysisResult analyzeFatEffects({
    ...
    try {
      ...
      // 5. 글루텐 네트워크 영향 분석
      final glutenEffects = _analyzeGlutenEffects(fatClassification, fatPercentage);
  ...
  /// 지방 타입별 분류
  static Map<String, dynamic> _classifyFatTypes(List<Map<String, dynamic>> fatIngredients) {
  ...
  static bool _isSolidFat(String name) {
    final solidFatKeywords = [
      '버터', '마가린', '쇼트닝', '라드', '우지', '코코넛오일',
      'butter', 'margarine', 'shortening', 'lard', 'tallow', 'coconut oil'
    ];
  ...
  static Map<String, dynamic> _analyzePhysicalProperties(
    ...
    if (solidFatWeight > 0) {
      if (fatPercentage >= 20.0) creamingAbility = 'excellent';
      else if (fatPercentage >= 10.0) creamingAbility = 'good';
      else creamingAbility = 'limited';
    }
    ...
    return {
      ...
      'plasticityIndex': _calculatePlasticityIndex(dominantType, fatPercentage, temperature),
    };
  }
  
  /// 가소성 지수 계산
  static double _calculatePlasticityIndex(String dominantType, double fatPercentage, double temperature) {
    if (dominantType != 'solid') return 0.0;
    
    // 온도에 따른 가소성 변화
    double temperatureFactor = 1.0;
    if (temperature > 25.0) temperatureFactor = 0.7; // 너무 따뜻하면 가소성 감소
  ...
  static Map<String, dynamic> _analyzeGlutenEffects(
    ...
    if (fatPercentage >= 20.0) shorteningEffect = 'strong';
    else if (fatPercentage >= 10.0) shorteningEffect = 'moderate';
    ...
    if (fatPercentage >= 20.0) volumeEffect = 'reduced';
  ...
  /// 총 지방 무게 계산
  static double _calculateTotalFatWeight(List<Map<String, dynamic>> fatIngredients) {
...
class FatAnalysisResult {
  ...
  String get fatTypeText {
    final dominantType = fatClassification['dominantFatType'] as String? ?? 'none';
    switch (dominantType) {
      case 'solid': return '고체지방 주도형';
      case 'liquid': return '액체지방 주도형';
      case 'mixed': return '혼합지방형';
      default: return '지방 없음';
    }
  }
  ...
  String get textureEffectText {
    ...
    switch (effect) {
      case 'flaky_tender': return '바삭하고 부드러움';
      case 'moist_tender': return '촉촉하고 부드러움';
      case 'tender': return '부드러움';
      case 'slightly_tender': return '약간 부드러움';
      default: return '보통';
    }
  }
  ...
  String get volumeEffectText {
    ...
    switch (effect) {
      case 'significantly_reduced': return '크게 감소';
      case 'reduced': return '감소';
      default: return '정상';
    }
  }

...
import '../services/fermentation_scenario_engine.dart';
import '../services/bread_calculator.dart';

// 실제 구현된 서비스 import (더미 클래스 대신)
...
// 모듈 서비스 import
import '../modules/bread/services/integrated_mixing_analyzer.dart';
import '../modules/bread/services/dough_type_analyzer.dart';
...
class RealTimeRecipeAnalyzer {
  ...
  static RecipeAnalysisResult analyzeIngredients(
    ...
    try {
      ...
      final saltPercentage = IngredientAnalyzer.calculateSaltPercentage(
              ...
              recipeTitle: recipeTitle) /
          100.0; // 소수점으로 변환
      final bakersPercentages = IngredientAnalyzer.calculateBakersPercentage(
          ingredients,
          recipeTitle: recipeTitle);
      ...
      final titleBasedBreadType = recipeTitle != null
          ...
          : 'general';
      final ingredientBasedBreadType = _estimateBreadTypeForCalculation(
          hydrationLevel, yeastPercentage, sugarIngredients, fatIngredients);
      final finalBreadType = titleBasedBreadType != 'general'
          ? titleBasedBreadType
          : ingredientBasedBreadType;

      // 반죽 상태 상세 분석 (종합 제빵 과학 통합 계산식 기반)
      // 발효 시간 추정: 이스트 비율에 따른 예상 발효 시간
      final estimatedFermentationTime = yeastPercentage > 0.01
          ? 90.0
          : // 표준 이스트: 90분
          yeastPercentage > 0.005
              ? 120.0
              : // 적은 이스트: 120분
              yeastPercentage > 0
                  ? 180.0
      ...
      final environmentalFactor = _calculateEnvironmentalFactor(
          environmentTemperature, environmentHumidity, altitude);

      // 특성 예측 (환경 보정 적용)
      final predictedTexture = _predictTexture(
          hydrationLevel, fatIngredients, eggIngredients, environmentalFactor);
      final predictedFlavor = _predictFlavor(sugarIngredients, fatIngredients,
          saltPercentage, environmentalFactor);
      final predictedAppearance = _predictAppearance(hydrationLevel,
          sugarIngredients, eggIngredients, environmentalFactor);

      // 베이킹 타입 추정 (제목 우선 로직)
      final estimatedBakingType = titleBasedBreadType != 'general'
          ? _getBreadTypeDisplayName(titleBasedBreadType)
          : _estimateBakingType(hydrationLevel, yeastPercentage,
              sugarIngredients, fatIngredients);
  ...
  static String _predictTexture(
    ...
    final adjustedHydration =
        hydrationLevel * (1 + (environmentalFactor - 1) * 0.1);
  ...
  static String _predictFlavor(
    ...
    final flavorIntensity = environmentalFactor > 1.2
        ? 1.2
        : (environmentalFactor < 0.8 ? 0.8 : environmentalFactor);
  ...
  static String _predictAppearance(
    ...
    // 환경 보정에 따른 수분 증발 조정
    final adjustedHydration =
        hydrationLevel * (environmentalFactor > 1.0 ? 0.95 : 1.05);
  ...
  static String _classifyNonYeastBaking(
      double hydrationLevel, double sugarWeight, double fatWeight) {
  ...
  static double _calculateConfidenceScore(
    ...
    // 필수 재료 존재 여부
    final hasFlour =
        IngredientAnalyzer.findFlourIngredients(ingredients).isNotEmpty;
    final hasLiquid =
        IngredientAnalyzer.findLiquidIngredients(ingredients).isNotEmpty;
...
class RecipeAnalysisResult {
  ...
  String get hydrationLevelText {
    if (hydrationLevel >= 0.75)
      return "고수분 (${(hydrationLevel * 100).toStringAsFixed(1)}%)";
    if (hydrationLevel >= 0.65)
      return "표준 (${(hydrationLevel * 100).toStringAsFixed(1)}%)";
    if (hydrationLevel >= 0.55)
      return "저수분 (${(hydrationLevel * 100).toStringAsFixed(1)}%)";
    return "매우 저수분 (${(hydrationLevel * 100).toStringAsFixed(1)}%)";
  }

  /// 이스트 비율 텍스트
  String get yeastPercentageText {
    if (yeastPercentage >= 0.02)
      return "높음 (${(yeastPercentage * 100).toStringAsFixed(1)}%)";
    if (yeastPercentage >= 0.01)
      return "표준 (${(yeastPercentage * 100).toStringAsFixed(1)}%)";
    if (yeastPercentage > 0)
      return "낮음 (${(yeastPercentage * 100).toStringAsFixed(1)}%)";
    return "없음";
  }
  ...
  String get saltPercentageText {
    if (saltPercentage >= 0.02)
      return "높음 (${(saltPercentage * 100).toStringAsFixed(1)}%)";
    if (saltPercentage >= 0.01)
      return "표준 (${(saltPercentage * 100).toStringAsFixed(1)}%)";
    if (saltPercentage > 0)
      return "낮음 (${(saltPercentage * 100).toStringAsFixed(1)}%)";
    return "없음";
  }
  ...
  String get fermentationProgressText {
    ...
    if (progress >= 0.8)
      return "충분히 발효됨 (${(progress * 100).toStringAsFixed(0)}%)";
    if (progress >= 0.5)
      return "적당히 발효됨 (${(progress * 100).toStringAsFixed(0)}%)";
    if (progress >= 0.2)
      return "발효 진행 중 (${(progress * 100).toStringAsFixed(0)}%)";
...
// Phase 5 확장 기능은 추후 별도 모듈로 분리 예정

...
class _RecipeListScreenState extends State<RecipeListScreen>
    ...
    provider.loadCategories().then((_) {
      if (mounted) {
        setState(() {
          ...
          _isLoading = false;
          print(
              'RecipeListScreen: Categories loaded. Count: ${provider.categories.length}');
          print(
              'RecipeListScreen: TabController initialized with length: ${_tabController?.length}');
        });
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading || _tabController == null) {
      print(
          'RecipeListScreen: Loading state - _isLoading: $_isLoading, _tabController == null: ${_tabController == null}');
        ...
        actions: [
          IconButton(
              ...
              onPressed: () => showSearch(
                  context: context,
                  delegate: RecipeSearchDelegate(provider.recipes))),
          IconButton(
      ...
      body: TabBarView(
        ...
        children: categories.map((category) {
          final filteredRecipes = provider.recipes
              ...
              .toList();
          print(
              "RecipeListScreen: Category '$category'. Filtered recipes count: ${filteredRecipes.length}");
          return filteredRecipes.isEmpty
              ? Center(
                  child: Text('$category 레시피를 추가하세요!',
                        ...
                        child: Column(
                          children: [
                            recipe.imagePath != null
                                ...
                                : Container(
                                    height: 200,
                                    child: Icon(Icons.dinner_dining, size: 50)),
                            ListTile(
                              ...
                              subtitle: FutureBuilder<int>(
                                ...
                                builder: (context, snapshot) {
                                  ...
                                  return Text(
                                      '${recipe.category}${derivedInfo}${recipe.parentId != null ? ' (파생됨)' : ''}');
                                },
                              ),
                            ),
                          ],
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () async {
          ...
          final newRecipe = await Navigator.push<Recipe?>(
              context,
              buildPageRoute<Recipe?>(
                  AddRecipeScreen(initialCategory: initialCategory)));
        },

...
class _AddRecipeScreenState extends State<AddRecipeScreen> {
  ...
  void initState() {
    ...
    if (widget.recipe != null) {
      print('AddRecipeScreen: initState - 레시피 정보 - ID: ${widget.recipe!.id}, 제목: ${widget.recipe!.title}, 부모 ID: ${widget.recipe!.parentId}, isCopy: ${widget.isCopy}, isDerivedCopy: ${widget.isDerivedCopy}');
      _title = widget.recipe!.title;
      _category = widget.recipe!.category;
      _ingredients = widget.recipe!.ingredients.map((ingredient) => ingredient.toJson()).toList();
      _instructions = widget.recipe!.instructions.map((instruction) => <String, dynamic>{
        'text': instruction,
        'imagePath': null,
      }).toList();
      _servingsController.text = widget.recipe!.baseServings.toString();
      _isBakingMode = widget.recipe!.isBaking;
      _splitAmountController.text = widget.recipe!.targetSplitAmount?.toString() ?? '';
      _splitCountController.text = widget.recipe!.targetSplitCount?.toString() ?? '';
      _splitAmountUnit = 'g'; // 기존 레시피 로드 시 기본값 설정
      
      // 발효 설정과 오븐 설정 불러오기
      if (widget.recipe!.fermentationSteps != null) {
        _fermentationSteps = List<Map<String, dynamic>>.from(widget.recipe!.fermentationSteps!);
      }
      if (widget.recipe!.ovenSteps != null) {
        _ovenSteps = List<Map<String, dynamic>>.from(widget.recipe!.ovenSteps!);
      }

      if (widget.isCopy || widget.isDerivedCopy) { // isCopy 또는 isDerivedCopy일 때 이미지 복사
        ...
        if (widget.isCopy && !widget.isDerivedCopy) { // 단순 복사일 경우에만 제목에 (복사본) 추가
          _title = '${_title} (복사본)';
        }
        ...
        _image = widget.recipe!.imagePath != null ? File(widget.recipe!.imagePath!) : null;
      }
    } else {
      _category = widget.initialCategory ?? Provider.of<RecipeProvider>(context, listen: false).categories[0];
      _instructions = [<String, dynamic>{'text': '', 'imagePath': null}];
  ...
  Future<void> _showImageSourceSelectionSheet(BuildContext context, {bool isInstructionImage = false, int? instructionIndex}) async {
                ...
                onTap: () async {
                  ...
                  if (isInstructionImage) {
                    ...
                    if (pickedFile != null) {
                      ...
                      if (compressedImage != null) {
                        setState(() {
                          _instructions[instructionIndex!]['imagePath'] = compressedImage.path;
                        });
                      }
                    }
                  } else {
  ...
  String? _getSplitCountHintText() {
    ...
    if (_splitAmountController.text.isNotEmpty) {
      ...
      final convertedSplitAmount = UnitConverter.convert(splitAmount, _splitAmountUnit!, 'g');
  ...
  void _onSplitCountChanged() {
    ...
    if (splitCount > 0 && totalWeight > 0) {
      ...
      _isProgrammaticChange = true;
      _splitAmountController.text = UnitConverter.convert(weightPerSplit, 'g', _splitAmountUnit!).toStringAsFixed(2);
      _isProgrammaticChange = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(widget.isCopy ? (widget.isDerivedCopy ? '파생 레시피 저장' : '레시피 복사') : (widget.recipe == null ? '레시피 추가' : '레시피 수정'))),
          ...
          child: Column(
            children: [
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: _image != null
                    ...
                    _image!,
                    fit: BoxFit.cover,
                    errorBuilder: (context, error, stackTrace) => Icon(Icons.broken_image, color: Colors.pink[100]),
                  )
              ...
              DropdownButtonFormField<String>(
                value: _category,
                items: provider.categories.map((String category) => DropdownMenuItem(value: category, child: Text(category))).toList(),
              ...
              if (!_isBakingMode)
                TextFormField(
                  ...
                  validator: (value) {
                    ...
                    if (servings == null || servings <= 0) return '유효한 인분 수를 입력하세요';
                    return null;
                  },
                )
              else
                      ...
                      children: [
                        Expanded(
                          ...
                          child: TextFormField(
                            ...
                            keyboardType: TextInputType.number,
                            onChanged: (value) { _onSplitAmountChanged(); },
                            validator: (value) {
                              if (value!.isEmpty && _splitCountController.text.isEmpty) return '분할량 또는 분할 개수를 입력하세요';
                              if (value.isNotEmpty) {
                                final splitAmount = double.tryParse(value);
                                if (splitAmount == null || splitAmount <= 0) return '유효한 분할량을 입력하세요';
                                final totalWeight = _calculateTotalIngredientWeight();
                                final convertedSplitAmount = UnitConverter.convert(splitAmount, _splitAmountUnit!, 'g');
                                if (convertedSplitAmount > totalWeight) {
                        ...
                        Expanded(
                          ...
                          child: DropdownButtonFormField<String>(
                            ...
                            items: UnitConverter.getUnits(provider.unitSystem).map((unit) => DropdownMenuItem(value: unit, child: Text(unit))).toList(),
                            onChanged: (value) {
                              ...
                              if (splitAmountText.isNotEmpty) {
                                final originalAmount = double.tryParse(splitAmountText) ?? 0.0;
                                final convertedAmount = UnitConverter.convert(originalAmount, oldUnit, newUnit);
                                
                                _isProgrammaticChange = true;
                                _splitAmountController.text = convertedAmount.toStringAsFixed(2);
                                _isProgrammaticChange = false;
                              }
                      ...
                      children: [
                        Expanded(
                          child: TextFormField(
                            ...
                            keyboardType: TextInputType.number,
                            onChanged: (value) { _onSplitCountChanged(); },
                            validator: (value) {
                              if (value!.isEmpty && _splitAmountController.text.isEmpty) return '분할량 또는 분할 개수를 입력하세요';
                              if (value.isNotEmpty) {
                                final splitCount = int.tryParse(value);
                                if (splitCount == null || splitCount <= 0) return '유효한 분할 개수를 입력하세요';
                              }
                              return null;
                            },
                          ),
              ...
              Row(
                children: [
                  Expanded(child: TextFormField(controller: _ingredientNameController, decoration: InputDecoration(labelText: '재료 이름'))),
                  SizedBox(width: 10),
                  Expanded(child: TextFormField(controller: _ingredientAmountController, decoration: InputDecoration(labelText: '양'), keyboardType: TextInputType.number)),
                  SizedBox(width: 10),
                  DropdownButton<String>(
                    value: _unit,
                    items: UnitConverter.getUnits(provider.unitSystem).map((unit) => DropdownMenuItem(value: unit, child: Text(unit))).toList(),
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              onChanged: (value) => _instructions[index]['text'] = value,
                          ...
                          IconButton(
                            ...
                            onPressed: () async {
                              _showImageSourceSelectionSheet(context, isInstructionImage: true, instructionIndex: index);
                            },
                          ),
                          IconButton(
                            icon: Icon(Icons.delete),
              ...
              TextButton.icon(
                icon: Icon(Icons.add),
                label: Text('단계 추가'),
                onPressed: () => setState(() => _instructions.add(<String, dynamic>{'text': '', 'imagePath': null})),
              ),
            ],
          ),
        ),
      ...
      floatingActionButton: isKeyboardVisible ? null : FloatingActionButton(
        onPressed: () async {
          if (_formKey.currentState!.validate()) {
            ...
            final double? targetSplitAmount = _splitAmountController.text.isNotEmpty
                ? UnitConverter.convert(double.tryParse(_splitAmountController.text) ?? 0.0, _splitAmountUnit!, 'g')
                : null;
            final int? targetSplitCount = int.tryParse(_splitCountController.text);
            final double totalIngredientWeight = _calculateTotalIngredientWeight();
            double? calculatedRemainingWeight;

            if (_isBakingMode) {
              if (targetSplitAmount != null && targetSplitAmount > 0) {
                final numSplits = (totalIngredientWeight / targetSplitAmount).floor();
                calculatedRemainingWeight = totalIngredientWeight - (numSplits * targetSplitAmount);
              } else if (targetSplitCount != null && targetSplitCount > 0) {
                calculatedRemainingWeight = totalIngredientWeight % targetSplitCount;
              }
            }
            ...
            if (widget.isDerivedCopy) {
              ...
              print('AddRecipeScreen: 파생 레시피 생성 모드 - 부모 ID: $finalParentId');
            } else if (widget.recipe != null && !widget.isCopy) {
              // 레시피 수정 모드 - 기존 parentId 유지
              finalParentId = widget.recipe!.parentId;
              print('AddRecipeScreen: 레시피 수정 모드 - 기존 부모 ID 유지: $finalParentId');
            } else {
              // 일반 레시피 생성 모드
              finalParentId = null;
            ...
            final recipe = Recipe(
              ...
              title: _title,
              category: _category,
              ingredients: _ingredients.map((ing) => Ingredient.fromJson(ing)).toList(),
              instructions: _instructions.map((inst) => (inst['text'] as String?) ?? '').toList(),
              imagePath: _image?.path,
              baseServings: _isBakingMode ? 1 : (int.tryParse(_servingsController.text) ?? 1),
              ...
              fermentationSteps: _isBakingMode && _fermentationSteps.isNotEmpty ? _fermentationSteps : null,
              ovenSteps: _isBakingMode && _ovenSteps.isNotEmpty ? _ovenSteps : null,
            );
            try {
              // 저장 조건 로깅
              print('AddRecipeScreen: 저장 조건 - recipe == null: ${widget.recipe == null}, isCopy: ${widget.isCopy}, isDerivedCopy: ${widget.isDerivedCopy}, recipe.id: ${recipe.id}, parentId: ${recipe.parentId}');
              ...
              if (recipe.id == null) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(widget.isCopy ? (widget.isDerivedCopy ? '파생 레시피가 저장되었습니다.' : '레시피가 복사되어 저장되었습니다.') : '레시피가 추가되었습니다.')),
                );
              } else {
                // ID가 있는 경우 (기존 레시피 수정)
                print('AddRecipeScreen: 기존 레시피 수정 - updateRecipe 호출 - ID: ${recipe.id}, 부모 ID: ${recipe.parentId}');
                try {
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.local_fire_department, color: Colors.orange.shade700, size: 20),
                ),
                SizedBox(width: 12),
                Text(
                  '오븐 설정',
                ...
                return Column(
                  children: [
                              ...
                              children: [
                                Expanded(
                                  child: Container(
                                    height: 40,
                                    child: TextFormField(
                                      initialValue: _ovenSteps[index]['temperature'].toString(),
                                      ...
                                      keyboardType: TextInputType.number,
                                      onChanged: (value) {
                                        _ovenSteps[index]['temperature'] = int.tryParse(value) ?? 180;
                                      },
                                    ),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: Container(
                                    height: 40,
                                    child: TextFormField(
                                      initialValue: _ovenSteps[index]['time'].toString(),
                                      style: TextStyle(fontSize: 14),
                                      decoration: InputDecoration(
                                        labelText: '시간 (분)',
                                        labelStyle: TextStyle(fontSize: 12),
                                      ...
                                      keyboardType: TextInputType.number,
                                      onChanged: (value) {
                                        _ovenSteps[index]['time'] = int.tryParse(value) ?? 30;
                                      },
                                    ),
                                  ),
                                ),
                    ...
                    Container(
                      margin: EdgeInsets.only(bottom: 8),
                      child: TextFormField(
                        initialValue: _ovenSteps[index]['comment']?.toString() ?? '',
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.bubble_chart, color: Colors.blue.shade700, size: 20),
                ),
                SizedBox(width: 12),
                Text(
                  '발효 설정',
                ...
                return Container(
                  ...
                  child: Column(
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: Container(
                              height: 40,
                              child: TextFormField(
                                initialValue: _fermentationSteps[index]['humidity'].toString(),
                                style: TextStyle(fontSize: 14),
                                decoration: InputDecoration(
                                  labelText: '습도 (%)',
                                  labelStyle: TextStyle(fontSize: 12),
                                ...
                                keyboardType: TextInputType.number,
                                onChanged: (value) {
                                  _fermentationSteps[index]['humidity'] = int.tryParse(value) ?? 80;
                                },
                              ),
                            ),
                          ),
                          ...
                          Expanded(
                            child: Container(
                              height: 40,
                              child: TextFormField(
                                initialValue: _fermentationSteps[index]['time'].toString(),
                                style: TextStyle(fontSize: 14),
                                decoration: InputDecoration(
                                  labelText: _fermentationSteps[index]['time'] == 0 || _fermentationSteps[index]['time'] == null 
                                      ? '시간을 분단위로 입력하세요' 
                                ...
                                onChanged: (value) {
                                  setState(() {
                                    _fermentationSteps[index]['time'] = _parseTimeInput(value);
                                  });
                                },
                              ),
                            ),
                          ...
                          Expanded(
                            child: Container(
                              height: 40,
                              child: TextFormField(
                                initialValue: _fermentationSteps[index]['temperature'].toString(),
                                style: TextStyle(fontSize: 14),
                                decoration: InputDecoration(
                                  labelText: '온도 (°C)',
                                  labelStyle: TextStyle(fontSize: 12),
                                ...
                                keyboardType: TextInputType.number,
                                onChanged: (value) {
                                  _fermentationSteps[index]['temperature'] = int.tryParse(value) ?? 28;
                                },
                              ),
                            ),
                          ),
                      ...
                      // 코멘트 입력 필드 추가
                      TextFormField(
                        initialValue: _fermentationSteps[index]['comment']?.toString() ?? '',
                        style: TextStyle(fontSize: 13),
                        decoration: InputDecoration(

...
class _FavoritePresetsScreenState extends State<FavoritePresetsScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      final favorites = await FavoritePresetService.instance.getSortedFavorites(_sortOption);
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              const PopupMenuItem(value: 'add_custom', child: Text('커스텀 프리셋 추가')),
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildEnvItem('온도', '${recommendation['environmentalConditions']['temperature']}°C', Icons.thermostat, Colors.red),
                      const SizedBox(width: 16),
                      _buildEnvItem('습도', '${recommendation['environmentalConditions']['humidity']}%', Icons.water_drop, Colors.blue),
                      const SizedBox(width: 16),
                      _buildEnvItem('고도', '${recommendation['environmentalConditions']['altitude']}m', Icons.landscape, Colors.green),
                    ],
                  ),
                ],
              ),
            ...
            Row(
              children: [
                ...
                const Spacer(),
                ElevatedButton(
                  onPressed: () => _createFavoriteFromRecommendation(recommendation),
        ...
        child: Column(
          ...
          children: [
            ...
            if (_stats!.topPerformers.isEmpty)
              ...
              ..._stats!.topPerformers.take(3).map((preset) => 
                ListTile(
                  ...
                  subtitle: Text('성공률: ${(preset.successRate * 100).toInt()}%'),
        ...
        child: Column(
          ...
          children: [
            ...
            if (_stats!.mostUsed.isEmpty)
              ...
              ..._stats!.mostUsed.take(3).map((preset) => 
                ListTile(
                  leading: CircleAvatar(
        ...
        child: Column(
          ...
          children: [
            ...
            ..._stats!.categoryDistribution.entries.map((entry) {
              final percentage = (entry.value / _stats!.totalPresets * 100).toInt();
              return Padding(
...
// 유틸리티 메서드들
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.of(context).pop();
              try {
                await FavoritePresetService.instance.deleteFavorite(favorite.id);
                _loadData();
  ...
  void _createFavoriteFromRecommendation(Map<String, dynamic> recommendation) async {
    try {
